...
void SetLocationX(const int value) {
  ...
  std::cout << "Contr Location X: " << sceneParam_->GetLocationX() << std::endl;
}

void SetLocationY(const int value) {
  sceneParam_->SetLocationY(value / 200.0);
  std::cout << "Contr Location Y: " << sceneParam_->GetLocationY() << std::endl;
}

void SetLocationZ(const int value) {
  sceneParam_->SetLocationZ(value / 200.0);
  std::cout << "Contr Location Z: " << sceneParam_->GetLocationZ() << std::endl;
}

void SetRotationX(const int value) {
  sceneParam_->SetRotationX(value);
  std::cout << "Contr Rotation X: " << sceneParam_->GetRotationX() << std::endl;
}

void SetRotationY(const int value) {
  sceneParam_->SetRotationY(value);
  std::cout << "Contr Rotation Y: " << sceneParam_->GetRotationY() << std::endl;
}

void SetRotationZ(const int value) {
  sceneParam_->SetRotationZ(value);
  std::cout << "Contr Rotation Z: " << sceneParam_->GetRotationZ() << std::endl;
}
...
void SetVertexColor(const QColor& color) {
  ...
  std::cout << "Contr VertexColor: r" << sceneParam_->GetVertexColor().r << " g"
            << sceneParam_->GetVertexColor().g << " b"
...
void SetBackgroundColor(const QColor& color) {
  sceneParam_->SetBackgroundColor({static_cast<float>(color.red() / 255.0),
                                   static_cast<float>(color.green() / 255.0),
                                   static_cast<float>(color.blue() / 255.0)});
  std::cout << "Contr BackgroundColor: r" << sceneParam_->GetBackgroundColor().r
            << " g" << sceneParam_->GetBackgroundColor().g << " b"
