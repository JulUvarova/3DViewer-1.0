...
inline void GifGetClosestPaletteColor(GifPalette* pPal, int r, int g, int b,
...
inline int GifPartition(uint8_t* image, const int left, const int right, const int elt,
                 int pivotValue) {
  int storeIndex = left;
  bool split = 0;
...
// median
inline void GifPartitionByMedian(uint8_t* image, int left, int right, int com,
                          int neededCenter) {
  if (left < right - 1) {
...
inline int GifPartitionByMean(uint8_t* image, int left, int right, int com,
                       int neededMean) {
  if (left < right - 1) {
...
inline void GifSplitPalette(uint8_t* image, int numPixels, int treeNode, int treeLevel,
...
// changed pixels only.
inline int GifPickChangedPixels(const uint8_t* lastFrame, uint8_t* frame,
                         int numPixels) {
  int numChanged = 0;
...
// technique
inline void GifMakePalette(const uint8_t* lastFrame, const uint8_t* nextFrame,
...
inline void GifDitherImage(const uint8_t* lastFrame, const uint8_t* nextFrame,
...
inline void GifThresholdImage(const uint8_t* lastFrame, const uint8_t* nextFrame,
                       uint8_t* outFrame, uint32_t width, uint32_t height,
                       GifPalette* pPal) {
...
inline void GifWriteLzwImage(FILE* f, uint8_t* image, uint32_t left, uint32_t top,
...
inline bool GifBegin(GifWriter* writer, const char* filename, uint32_t width,
...
inline bool GifWriteFrame(GifWriter* writer, const uint8_t* image, uint32_t width,
