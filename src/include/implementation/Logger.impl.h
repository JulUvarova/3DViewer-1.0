...
namespace {
  ...
  // Setters
  inline void Logger::setMaxLogLevel(const Logger& logger_ [[maybe_unused]]){
  ...
  inline std::string Logger::getPrefixString(const Logger& loggerConstructor [[maybe_unused]]){
  ...
  inline Logger &Logger::operator<<(Logger& l_ [[maybe_unused]]){
    return *this;
  }
