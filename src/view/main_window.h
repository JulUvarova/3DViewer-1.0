...
class MainWindow : public QMainWindow {
 ...
 private:
  ...
  SlidersBox *locationSlidersBox_, *rotateSlidersBox_, *scaleSlidersBox_; ///< Sliders for transformations
  ...
  QPushButton *resetCoordsButton_, *resetElemsButton_, *saveElemsButton_, *restoreElemsButton_, *sceneInfoButton_; ///< Buttons for various actions
  QRadioButton *perspectiveProj_, *parallelProj_; ///< Radio buttons for projection type
  InfoWindow *sceneInfoWindow_; ///< Info window for displaying scene information

  // Controller
  std::shared_ptr<s21::Controller> controller_; ///< Controller for managing application logic

  // User GUI settings
  std::shared_ptr<UserSetting> userSetting_; ///< User settings for the application

  // For saving
  std::vector<QPixmap> screens_; ///< Vector to hold screenshots for GIF creation
